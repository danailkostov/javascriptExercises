...
function iqTest(numbers){
    //split string to even and odd elements
    let even = numbers.split(' ').filter(num => num % 2 === 0);
    let odd = numbers.split(' ').filter(num => num % 2 !== 0);
    
    //calculate where exactly is the difference
      if(odd.length === 1) {
        return numbers.split(' ').indexOf(odd[0]) + 1;
      } else {
        return numbers.split(' ').indexOf(even[0]) + 1;
      }
  }
