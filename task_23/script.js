...
function findEvenIndex(arr) {
  for (let arrayIndex = 0; arrayIndex < arr.length; arrayIndex += 1) {
    let firstInt = 0;
    let secondInt = 0;
    for (let leftIndex = 0; leftIndex < arrayIndex; leftIndex += 1) {
      firstInt += arr[leftIndex];
    }
    for (
      let rightIndex = arrayIndex + 1;
      rightIndex < arr.length;
      rightIndex += 1
    ) {
      secondInt += arr[rightIndex];
    }
    if (firstInt === secondInt) return arrayIndex;
  }
  return -1;
}
