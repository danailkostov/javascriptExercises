...
function dropElements(arr, func) {
  // find first met func conditions element
  const findEl = arr.find(func);
  // find index of the element
  const elIndex = arr.indexOf(findEl);
  let updatedArr;
  // update the arr
  if (elIndex === -1) {
    updatedArr = [];
  } else {
    updatedArr = arr.slice(elIndex);
  }
  return updatedArr;
}
...
// dropElements([0, 1, 0, 1], (n) => n === 1); shoudl return 1,0,1 but returns 1,1;
